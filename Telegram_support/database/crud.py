...
def update_user_jira_issue_key(telegram_id: int, jira_issue_key: str) -> bool:
    """
    Оновлює jira_issue_key для користувача.

    Args:
        telegram_id: telegram_user_id користувача
        jira_issue_key: ключ Jira issue (наприклад, 'TP-17')
    ...
    try:
        with Session(engine) as session:
            ...
            # Оновлюємо jira_issue_key
            user.jira_issue_key = jira_issue_key
            session.add(user)
            session.commit()

            logger.info(f"✅ Jira issue key updated for user {telegram_id}: {jira_issue_key}")
            return True
